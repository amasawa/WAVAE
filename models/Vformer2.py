...
class Vformer(nn.Module):
    def __init__(self, args):
        ...
        self.phi_enc = nn.Sequential(
            nn.Linear((self.configs.seq_len + self.configs.label_len)*self.configs.d_model, self.configs.h_dim),
            nn.ReLU(),
            nn.Linear(self.configs.h_dim, self.configs.h_dim),
            nn.ReLU())
        ...
        self.dec_std = nn.Sequential(
            nn.Linear(self.configs.h_dim, (self.configs.seq_len + self.configs.label_len)*self.configs.x_dim),
            nn.Softplus()).cuda()
    ...
    def forward(self,x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        ## VAE part
        y_encoder, attns = self.encoder(dec_emd, attn_mask=enc_self_mask)

        # VAE_Prior
        prior = self.phi_prior(enc_out.reshape(-1,self.configs.pred_len*self.configs.d_model))
        prior_mean = self.prior_mean(prior)  # [64, 16]
        prior_std = self.prior_std(prior)    # [64, 16]
        # print(prior_mean.shape)
        # print(prior_std.shape)

        # VAE_Encoder
        enc = self.phi_enc(y_encoder.reshape(-1,(self.configs.seq_len+ self.configs.label_len)*self.configs.d_model))
        ...
        dec_mean = self.dec_mean(dec).reshape(-1,(self.configs.seq_len + self.configs.label_len),x_enc.shape[2])
        dec_std = self.dec_std(dec).reshape(-1,(self.configs.seq_len + self.configs.label_len),x_enc.shape[2])
        ...
        if self.configs.loss_function == 'nll':
            nll_loss = self.nll_gaussian_1(mean=dec_mean, std=dec_std, x = x_dec)
        elif self.configs.loss_function == 'mse':
            nll_loss = self.mse(mean=dec_mean, x = x_dec)
        elif self.configs.loss_function == 'beta_1':
            nll_loss = self.beta_gaussian_1(mean=dec_mean, x=x_dec, beta=self.configs.robust_coeff)
        elif self.configs.loss_function == 'beta_2':
            nll_loss = self.beta_gaussian_2(mean=dec_mean, std=dec_std, x=x_dec, beta=self.configs.robust_coeff)
    ...
    def mse(self, mean, x):
        REC = torch.nn.functional.mse_loss(mean, x, size_average=False).div(x.shape[0])
        # print(REC.item())
        # assert False
        return REC
